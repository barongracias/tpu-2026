# R7 K8 Full-Test Evaluation Log - 2026-06-14
...
- Purpose: confirm the selected R7 RLOO K=8 best retained checkpoint on the full GSM8K test split.
- Decision this run should enable: report-grade full-test accuracy and paired bootstrap CI for base vs R7 K=8 best retained checkpoint.

## Source State

- `tpu-2026` run commit: `e3d69a1938fe8e8a2a67a3c84a03331133ed46f1`
- Current repo status before launch: clean apart from this untracked iteration log.
...
- Step `3364` final checkpoint was not full-evaluated in this pass.
...
python -u experiments/analysis/bootstrap_ci.py   --csv base="$RUN_ROOT/eval/base_full.csv"   --csv r7_k8_best="$RUN_ROOT/eval/r7_rloo_k8_step2000_full.csv"   --baseline base   --resamples 10000 --seed 12345   --output-json "$RUN_ROOT/eval/r7_rloo_k8_best_full_ci.json"   --output-csv "$RUN_ROOT/eval/r7_rloo_k8_best_full_ci.csv"
```
...
Step `3364` was not full-evaluated. A launch attempt after step `2000` hit an
active TPU lock from another user's eval queue, and the operator stopped rather
than spend more shared TPU time.
