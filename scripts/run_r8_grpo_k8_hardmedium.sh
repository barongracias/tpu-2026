...
export RUN_ROOT=${RUN_ROOT:-"$HOME/tpu-runs/part-i/R8-grpo-k8-hardmedium-$(date +%Y%m%d_%H%M%S)"}
export WANDB_NAME=${WANDB_NAME:-R8-grpo-k8-hardmedium}
...

{
  ...
  echo "SAVE_INTERVAL_STEPS=$SAVE_INTERVAL_STEPS"
  echo "MAX_TO_KEEP=$MAX_TO_KEEP"
} | tee "$RUN_ROOT/logs/launch_env.txt"
